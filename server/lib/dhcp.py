...
class DHCPManager(AdHocManager):
    ...
    def transfer_from_old_database(self, fun, optionset_manager):
        ...
        for(my_id, authoritative, info, changed_by, mtime) in self.odb.get(qf):
            ...
            self.db.insert("id", qp, id=my_id, authoritative=authoritative, info=info,
                           changedby=changed_by, mtime=mtime, optset=optset)
        ...
        for(my_id, group, mac, room, optionspace, changed_by, mtime, info, entry_status, cid) in self.odb.get(qf):
            ...
            # Check that the resulting room code matches the defined regexp. If not, issue warning and skip the room
            import re
            if room:
        ...
        # the code above. However, the actual usage must be transferred into the
        # xxx_options tables from the classes, groups etc tables.
    ...
    def make_dhcpd_conf(self, serverID=None):
        ...
        self.emit("# dhcpd.conf - Rev: %d Automatically generated for DHCP server %s by AdHoc server %s (svn %s). Do not edit!" %
                  (eventid, serverID, adhoc_release, adhoc_svn_version), 0)
    ...
    def emit_host(self, hostid, dns, mac, room, entry_status, optionspace, info, hasopts, indent):
        ...
        if entry_status == "Inactive":
            if dns:
                self.emit("#host %s { hardware ethernet %s; fixed-address %s;} # %s,  %s" %
                          (hostid, mac, dns, entry_status, comment), 4 * indent - 1)
            else:
                self.emit("#host %s { hardware ethernet %s;} # %s,  %s" %
                          (hostid, mac, entry_status, comment), 4 * indent - 1)
            return
    ...
    def emit_class(self, classname, optionspace, vendor_class_id):
        ...
        if vendor_class_id:
            ...
            self.emit("match if substring (option vendor-class-identifier, 0, %d) = \"%s\";" %
                      (length, vendor_class_id), 4)
    ...
    def emit_optlist(self, object, indent):
        ...
        for name in option_names:
            ...
            (space, opt_type, qual) = self.db.get_all(
                "SELECT optionspace, type, qualifier FROM option_base WHERE name = :name ", name=name)[0]
            opt = ""
