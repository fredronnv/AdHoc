...
class Host(AdHocModel):
    ...
    def set_group(self, new_group):
        ...
        self.event_manager.add(
            "disconnect", host=self.oid, parent_object=self.group, authuser=self.function.session.authuser)
        self.event_manager.add(
            "connect", host=self.oid, parent_object=new_group.oid, authuser=self.function.session.authuser)
...
class HostManager(AdHocManager):
    ...
    def add_literal_option(self, fun, host, option_text):
        ...
        self.approve()
        self.event_manager.add("create", host=host.oid, literal_option_id=id, literal_option_value=unicode(
            option_text), authuser=fun.session.authuser)
        return id
    ...
    def update_options(self, fun, host, updates):
        ...
        for (key, value) in updates.iteritems():
            ...
            self.event_manager.add(
                "update", host=host.oid, option=key, option_value=unicode(value), authuser=fun.session.authuser)
