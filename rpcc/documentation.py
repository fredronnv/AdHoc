...
"""
    ...
    def function_list_as_html(self, apivers):
        api = self.server.api_handler.get_api(apivers)
        doc = '<html><head><title>Listing of available functions for API %s</title>' % (api.get_version_string())
        doc += '<link rel="stylesheet" type="text/css" href="/api.css">'
        doc += '</head><body>'
        doc += '<h1>List of all available functions in API %s</h1><ul>' % (api.get_version_string())
        for name in api.get_visible_function_names():
            doc += '<li><a href="/api/%d/%s">%s()</a>' % (apivers, name, name)
        doc += '</ul></body></html>'
        return doc
